...
// The registry ships ahead of @assistant-ui/react 0.15.17: the approval
// `display`/`prompt` fields and the `toolApprovalAcceptsText` helper are not in
// the published types yet. These shims keep the file compiling; drop them once
...
const asApproval = (approval: ToolCallMessagePart["approval"]) =>
  approval as ToolApproval | undefined;

const toolApprovalAcceptsText = (approval: ToolCallMessagePart["approval"]) =>
  asApproval(approval)?.display === "text";
...
const isQuestion = (approval: ToolCallMessagePart["approval"]) =>
  asApproval(approval)?.display === "select" ||
  asApproval(approval)?.display === "text";
...
function ToolFallbackApproval({
  ...
  const typedAnswer = () =>
    answer.trim() ? ({ text: answer } as Partial<ToolApprovalResponse>) : {};

  const submitAnswer = () => {
    if (submitted || !answer.trim()) return;
    submit(() =>
      respondToApproval?.({ text: answer } as unknown as ToolApprovalResponse),
    );
  };
  ...
  const prompt = asApproval(approval)?.prompt;
  ...
  const answerField = acceptsText ? (
    <div className="aui-tool-fallback-approval-answer flex flex-col items-start gap-2">
      <Textarea
        ...
        disabled={submitted}
        aria-label={question ? (prompt ?? "Answer") : "Note"}
        placeholder={
